function TrackObjectUI(button, container, videoframe, job, player, tracks)
{
    ...
    this.setup = function()
    {
        ...
        var html = "<p>In this image, please label all these joints if visible:</p>";
        html += "<ul>";
...
function TrackObject(job, player, container, color)
{
    ...
    {
        var html = "<p>Click on one of these joints:</p>";
    ...
    this.setupdetails = function()
    {
        ...
        $("#trackobject" + this.id + "delete").click(function() {
            if (window.confirm("Delete the " + me.job.labels[me.label].replace(/_/g, " ") + " annotation?"))
            {
                me.remove();
